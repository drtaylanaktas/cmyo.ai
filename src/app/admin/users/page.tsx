...
export default function UsersPage() {
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] gap-6 animate-in fade-in duration-500">
            ...
            <div className="w-1/3 bg-slate-900/40 border border-slate-800/60 rounded-2xl overflow-hidden backdrop-blur-xl shadow-2xl flex flex-col">
                <div className="p-4 border-b border-slate-800/60 bg-slate-900/50">
                    <div className="flex justify-between items-center mb-3">
                        <h3 className="font-semibold text-slate-200">Kayıtlı Kullanıcılar</h3>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-800/50 border border-slate-700/50 rounded-lg pl-8 pr-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500/50 transition-colors"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="py-20 text-center text-slate-500 text-sm">Kullanıcı bulunamadı.</div>
                    ) : (
                        users.map((user) => (
                            <div
                                key={user.id}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${selectedUser?.id === user.id ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-400'}`}>
                                        <User size={14} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-sm font-medium truncate ${selectedUser?.id === user.id ? 'text-blue-400' : 'text-slate-200'}`}>
                                            {user.name} {user.surname}
                                        </p>
                                        <p className="text-xs text-slate-500 truncate">{user.email}</p>
                                    </div>
                                    <div className="flex flex-col items-end gap-1 shrink-0">
                                        <span className="text-[10px] text-slate-500">{roleLabel(user.role)}</span>
            ...
            {/* Right: Detail Panel */}
            <div className="w-2/3 bg-slate-900/40 border border-slate-800/60 rounded-2xl overflow-hidden backdrop-blur-xl shadow-2xl flex flex-col">
                {!selectedUser ? (
                    ...
                    <>
                        <div className="p-4 border-b border-slate-800/60 bg-slate-900/50 flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-slate-200 truncate">{selectedConv.title}</p>
                                <p className="text-xs text-slate-500">{selectedUser.name} {selectedUser.surname} · {selectedConv.message_count} mesaj</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-950/30">
                            {msgsLoading ? (
                                ...
                                <div className="text-center text-slate-500 text-sm py-10">Bu sohbette mesaj yok.</div>
                            ) : (
                                <AnimatePresence initial={false}>
                                    {messages.map((msg, idx) => (
                                        ...
                                        >
                                            <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center ${msg.role === 'user' ? 'bg-slate-800 text-slate-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                                {msg.role === 'user' ? <User size={14} /> : <Bot size={14} />}
                                            </div>
                                            <div className={`flex flex-col max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                                <div className={`px-4 py-3 rounded-2xl text-[15px] leading-relaxed ${msg.role === 'user' ? 'bg-slate-800 text-slate-200 rounded-tr-sm' : 'bg-blue-900/30 border border-blue-500/20 text-blue-50 rounded-tl-sm'}`}>
                    ...
                    <>
                        <div className="p-5 border-b border-slate-800/60 bg-slate-900/50">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {selectedUser.title && (
                                        <p className="text-sm text-slate-400 mt-0.5">{selectedUser.title}</p>
                                    )}
                        ...
                        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800/40">
                            <h4 className="text-sm font-semibold text-slate-300">Sohbet Geçmişi</h4>
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-blue-500/15 text-blue-300 border border-blue-500/30 hover:bg-blue-500/25 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                            ...
                            {convsLoading ? (
                                ...
                                conversations.map((conv) => (
                                    <div
                                        ...
                                        className="p-3 rounded-xl cursor-pointer hover:bg-slate-800/50 border border-transparent hover:border-slate-700/50 transition-all duration-200 flex items-center gap-3"
                                    >
                                        <div className="w-8 h-8 rounded-lg bg-slate-800 text-slate-500 flex items-center justify-center shrink-0">
                                            <MessageCircle size={14} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm text-slate-200 font-medium truncate">{conv.title}</p>
                                            <p className="text-xs text-slate-500 mt-0.5">{conv.message_count} mesaj</p>
                                        </div>
